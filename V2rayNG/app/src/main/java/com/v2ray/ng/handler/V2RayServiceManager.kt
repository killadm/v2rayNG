...
import com.v2ray.ng.contracts.ServiceControl
import com.v2ray.ng.dto.ProfileItem
import com.v2ray.ng.enums.EConfigType
import com.v2ray.ng.extension.toast
import com.v2ray.ng.service.V2RayProxyOnlyService
import com.v2ray.ng.service.V2RayVpnService
import com.v2ray.ng.util.LogUtil
import com.v2ray.ng.util.MessageUtil
...
object V2RayServiceManager {
    ...
    private var routingWifiMonitorConnectivity: ConnectivityManager? = null
    @Volatile
    private var routingWifiStateFingerprint: String = ""
    @Volatile
    private var routingWifiRestartInProgress = false

    private val routingWifiNetworkRequest by lazy {
    ...
    private val routingWifiNetworkCallback = object : ConnectivityManager.NetworkCallback() {
        override fun onAvailable(network: Network) {
            handleRoutingWifiStateChanged()
        }
...
//        if (!result.status) return
    ...
    private fun registerRoutingWifiMonitor(service: Service) {
        unregisterRoutingWifiMonitor()

        if (!SettingsManager.hasWifiConditionalRoutingRules()) {
            routingWifiStateFingerprint = ""
            routingWifiRestartInProgress = false
            return
        }

        routingWifiMonitorConnectivity = service.getSystemService(ConnectivityManager::class.java)
        routingWifiStateFingerprint = SettingsManager.getRoutingRulesetWifiStateFingerprint(service)

        try {
            routingWifiMonitorConnectivity?.requestNetwork(routingWifiNetworkRequest, routingWifiNetworkCallback)
            routingWifiRestartInProgress = false
        } catch (e: Exception) {
            LogUtil.e(AppConfig.TAG, "StartCore-Manager: Failed to register routing Wi-Fi monitor", e)
        }
    }

    private fun unregisterRoutingWifiMonitor() {
        try {
            routingWifiMonitorConnectivity?.unregisterNetworkCallback(routingWifiNetworkCallback)
        } catch (e: Exception) {
            LogUtil.w(AppConfig.TAG, "StartCore-Manager: Failed to unregister routing Wi-Fi monitor", e)
        } finally {
            routingWifiMonitorConnectivity = null
        }
    }

    private fun handleRoutingWifiStateChanged() {
        val service = getService() ?: return
        if (!coreController.isRunning || routingWifiRestartInProgress || !SettingsManager.hasWifiConditionalRoutingRules()) {
            return
        }

        val currentFingerprint = SettingsManager.getRoutingRulesetWifiStateFingerprint(service)
        if (currentFingerprint == routingWifiStateFingerprint) {
            return
        }

        LogUtil.i(
            AppConfig.TAG,
            "StartCore-Manager: Routing Wi-Fi state changed ($routingWifiStateFingerprint -> $currentFingerprint), restarting service"
        )
        routingWifiStateFingerprint = currentFingerprint
        restartRunningService(service)
    }

    private fun restartRunningService(service: Service) {
        val serviceControl = serviceControl?.get() ?: return
        routingWifiRestartInProgress = true
    ...
    fun startCoreLoop(vpnInterface: ParcelFileDescriptor?): Boolean {
        ...
        try {
            ...
            registerRoutingWifiMonitor(service)
        ...
        override fun onReceive(ctx: Context?, intent: Intent?) {
            ...
            when (intent?.getIntExtra("key", 0)) {
                ...
                AppConfig.MSG_STATE_RESTART -> {
                    ...
                    restartRunningService(serviceControl.getService())
                }
