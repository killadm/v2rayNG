...
import com.v2ray.ng.AppConfig.TAG_PROXY
import com.v2ray.ng.R
import com.v2ray.ng.databinding.ActivityRoutingEditBinding
...
import com.v2ray.ng.util.WifiSsidUtil
import kotlinx.coroutines.Dispatchers
import kotlinx.coroutines.launch

class RoutingEditActivity : HelperBaseActivity() {
    ...
    private fun getSelectedProcessPackages(): List<String> {
        return parseCommaSeparatedList(binding.etProcess.text).orEmpty()
    }

    private fun setupCurrentWifiFill() {
        binding.btnFillCurrentWifi.setOnClickListener {
            checkAndRequestPermission(PermissionType.WIFI_SSID) {
                val currentWifi = WifiSsidUtil.resolveCurrentWifiSsid(this)
                when (currentWifi.status) {
                    WifiSsidUtil.CurrentWifiSsidStatus.AVAILABLE -> {
                        val ssid = currentWifi.ssid ?: return@checkAndRequestPermission
                        val updatedSsids = (WifiSsidUtil.parseSsidList(binding.etDisableOnWifiSsids.text.toString()).orEmpty() + ssid)
                            .distinct()
                        binding.etDisableOnWifiSsids.text = Utils.getEditable(updatedSsids.joinToString(","))
                    }

                    WifiSsidUtil.CurrentWifiSsidStatus.NOT_CONNECTED -> toast(R.string.routing_settings_current_wifi_not_connected)
                    WifiSsidUtil.CurrentWifiSsidStatus.LOCATION_DISABLED -> toast(R.string.routing_settings_current_wifi_location_disabled)
                    WifiSsidUtil.CurrentWifiSsidStatus.PERMISSION_REQUIRED,
                    WifiSsidUtil.CurrentWifiSsidStatus.UNAVAILABLE -> toast(R.string.routing_settings_current_wifi_unavailable)
                }
            }
        }
    }

    private fun parseCommaSeparatedList(text: CharSequence?): List<String>? {
        val raw = text?.toString().nullIfBlank() ?: return null
        return raw
            .split(',', '\n')
            .map { it.trim() }
    ...
    private fun bindingServer(rulesetItem: RulesetItem): Boolean {
        ...
        binding.etDisableOnWifiSsids.text = Utils.getEditable(rulesetItem.disableOnWifiSsids?.joinToString(","))
    ...
    private fun clearServer(): Boolean {
        ...
        binding.etDisableOnWifiSsids.text = null
        return true
    }
    ...
    private fun saveServer(): Boolean {
        ...
        rulesetItem.apply {
            ...
            domain = parseCommaSeparatedList(binding.etDomain.text)
            ip = parseCommaSeparatedList(binding.etIp.text)
            process = parseCommaSeparatedList(binding.etProcess.text)
            protocol = parseCommaSeparatedList(binding.etProtocol.text)
            port = binding.etPort.text.toString().nullIfBlank()
            network = binding.etNetwork.text.toString().nullIfBlank()
            disableOnWifiSsids = WifiSsidUtil.parseSsidList(binding.etDisableOnWifiSsids.text.toString())
