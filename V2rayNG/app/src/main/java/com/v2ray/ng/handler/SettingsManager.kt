...
import com.v2ray.ng.AppConfig.ANG_PACKAGE
import com.v2ray.ng.AppConfig.DEFAULT_SUBSCRIPTION_ID
import com.v2ray.ng.AppConfig.GEOIP_PRIVATE
import com.v2ray.ng.AppConfig.GEOSITE_PRIVATE
import com.v2ray.ng.AppConfig.TAG_DIRECT
import com.v2ray.ng.AppConfig.VPN
import com.v2ray.ng.dto.ProfileItem
import com.v2ray.ng.dto.RulesetItem
import com.v2ray.ng.dto.SubscriptionItem
import com.v2ray.ng.dto.V2rayConfig
import com.v2ray.ng.enums.EConfigType
import com.v2ray.ng.enums.Language
import com.v2ray.ng.enums.RoutingType
import com.v2ray.ng.enums.VpnInterfaceAddressConfig
import com.v2ray.ng.handler.MmkvManager.decodeAllServerList
import com.v2ray.ng.handler.MmkvManager.decodeServerConfig
import com.v2ray.ng.handler.MmkvManager.decodeSubsList
import com.v2ray.ng.handler.MmkvManager.decodeSubscription
import com.v2ray.ng.handler.MmkvManager.encodeSubscription
import com.v2ray.ng.handler.MmkvManager.removeSubscription
...
object SettingsManager {
    ...
    fun hasWifiConditionalRoutingRules(): Boolean {
        return MmkvManager.decodeRoutingRulesets()?.any { !it.disableOnWifiSsids.isNullOrEmpty() } == true
    }

    fun getEffectiveRoutingRulesets(context: Context): List<RulesetItem> {
        val rulesetItems = MmkvManager.decodeRoutingRulesets().orEmpty()
        if (rulesetItems.isEmpty()) {
            return emptyList()
        }

        val currentWifiSsid = WifiSsidUtil.resolveCurrentWifiSsid(context).ssid
        return rulesetItems.filterNot { item ->
            WifiSsidUtil.matches(currentWifiSsid, item.disableOnWifiSsids)
        }
    }

    fun getRoutingRulesetWifiStateFingerprint(context: Context): String {
        val currentWifiSsid = WifiSsidUtil.resolveCurrentWifiSsid(context).ssid
        return MmkvManager.decodeRoutingRulesets()
            .orEmpty()
            .mapIndexedNotNull { index, item ->
                index.takeIf {
                    item.enabled && WifiSsidUtil.matches(currentWifiSsid, item.disableOnWifiSsids)
                }
            }
            .joinToString(",")
    }
     ...
     */
    fun routingRulesetsBypassLan(context: Context): Boolean {
        ...
        val rulesetItems = getEffectiveRoutingRulesets(context)
